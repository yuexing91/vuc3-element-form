...
export default {
  ...
  computed: {
    formNode() {
      return this.vucAst.querySelector('el-form');
    },
  },
  watch: {
    vucAst: {
      handler(vucAst) {
        ...
        this.type = map.FormItem > map.GridFormItem ? 'FormItem' : 'GridFormItem';
      },
      immediate: true,
    },
  },
  methods: {
    ...
    changeToGridFormItem() {
      ...
      parents.forEach(parent => {
        let row = createVucNode(`<el-row></el-row>`);
        let formItems = map.get(parent);
        formItems[0].insertBefore(row);
        let cols = formItems.map(formItem => {
          let col = createVucNode(`<el-col :span="12"></el-col>`);
